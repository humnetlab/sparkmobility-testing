...
def remove_redundant_stays_parquet(input_path, output_path):
    ...
    # Drop helper columns
    df_filtered = df_filtered.drop(['location_id', 'prev_user', 'prev_location'], axis=1)
...
def extract_frequent_users_parquet(input_path, output_path, num_stays_threshold=15):
    ...
    frequent_users = user_location_counts[user_location_counts['num_locations'] > num_stays_threshold]['caid']
...
def extract_stay_regions_for_frequent_users_parquet(fa_users_path, input_path, output_path):
    """
    ...
    df_filtered['location_id'] = (df_filtered['Longitude'].astype(str) + '_' + 
                                 df_filtered['Latitude'].astype(str))
    ...
    df_final['date'] = pd.to_datetime(df_final['timestamp'], unit='s').dt.strftime('%Y-%m-%d')
    df_final['time'] = pd.to_datetime(df_final['timestamp'], unit='s').dt.hour + \
