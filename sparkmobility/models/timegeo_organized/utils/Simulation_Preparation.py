        ...
        for _, row in df.iterrows():
            ...
            if current_user is not None and user_id != current_user:
                ...
                for loc_id in user_locations:
                    g.write(f"{loc_id} {user_locations[loc_id]} "
    ...
    with open(output_path, 'w') as g:
        ...
        with open(commuter_path, 'r') as f:
            for line in f:
                ...
                # Write output line
                g.write(f"{user_id} {b1} {b2} {nw} {reg} {work_flag} {home_tract} {work_tract} {work_slots_str}\n")
...
def split_simulation_inputs(
            ...
            if len(parts) == 1:  # User ID line
                ...
                if current_user and len(parts) >= 4:
                    try:
                        ...
                        user_locations[current_user].append([loc_type, frequency, lon, lat])
    ...
    print(f"Splitting {len(valid_users_list)} users into {num_cpus} files with ~{chunk_size} users each")
...
def otherLocations(
    ...
    if not coordinates:
        print("Warning: No valid coordinates found in input file. Generating default coordinates.")
