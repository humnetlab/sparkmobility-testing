...
def consolidate_partitioned_data(input_path, output_path):
    ...
    if os.path.isdir(input_path):
        ...
        print(
            f"Loaded {len(df)} records from {len([f for f in os.listdir(input_path) if f.endswith('.parquet')])} partition files"
        )
...
def main_workflow(input_parquet_path, num_cpus=16):
    ...
    noncommuter_param_file = (
        "./results/Parameters/NonCommuters/ParametersNonCommuters.txt"
    )

    # Get file sizes
    commuter_size = (
        os.path.getsize(commuter_param_file)
        if os.path.exists(commuter_param_file)
        else 0
    )
    noncommuter_size = (
        os.path.getsize(noncommuter_param_file)
        if os.path.exists(noncommuter_param_file)
        else 0
    )

    print(
        f"Parameter file sizes: Commuters={commuter_size}, NonCommuters={noncommuter_size}"
    )

    # Verify we have both commuter and non-commuter parameters
    if commuter_size == 0:
        print(
            "⚠️  Warning: No commuter parameters found - this will result in no work locations!"
        )
    ...
    if commuter_size > 0 or noncommuter_size > 0:
        ...
        final_comm_size = (
            os.path.getsize("./results/Parameters/ParametersCommuters.txt")
            if os.path.exists("./results/Parameters/ParametersCommuters.txt")
            else 0
        )
        final_noncomm_size = (
            os.path.getsize("./results/Parameters/ParametersNonCommuters.txt")
            if os.path.exists("./results/Parameters/ParametersNonCommuters.txt")
            else 0
        )
        print(
            f"Decoded parameter files: Commuters={final_comm_size}, NonCommuters={final_noncomm_size}"
        )
    ...
    for result_file in result_files:
        if os.path.getsize(result_file) > 0:
            with open(result_file, "r") as f:
                ...
                print(
                    f"  {os.path.basename(result_file)}: {user_count} users simulated"
                )
        else:
    ...
    if total_users_simulated == 0:
        print(
            "ERROR: No users were simulated! Check the user ID alignment and parameter files."
        )
        return False
