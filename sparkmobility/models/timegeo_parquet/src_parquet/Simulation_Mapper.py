        ...
        with open(param_file, "r") as f:
            for line in f:
                ...
                if workingStatus:
                    ...
                    workingSlots = [
                        [b, b + dur]
                        for d, b, dur in zip(workingDays, beginSlots, durations)
                        if d
                    ]
                ...
                try:
                    locs, person_loc = processUser(
                        locsVisited, b1, b2, nw, modifiedWorkingSlots, workLocation
                    )
                except Exception:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            ...
            try:
                ...
                print(
                    f"Completed {completed}/{len(active_indices)} ({progress:.1f}%) - "
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                print(
                    f"Completed batch {completed}/{len(batches)} ({progress:.1f}%) - "
